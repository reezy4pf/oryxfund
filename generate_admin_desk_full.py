...
<body>
  ...
  <!-- Left Sidebar (Pixel-Perfect with User Screenshot) -->
  <aside class="desk-sidebar">
    
    <!-- Brand Header -->
    <a href="javascript:void(0)" class="desk-brand-header" onclick="switchView('dashboard')">
    ...
    <!-- Search Box (Ctrl+K) -->
    <div class="sidebar-search-box" onclick="openSearchModal()">
      <span>🔍 Search</span>
      <span class="kbd-shortcut">Ctrl+K</span>
    </div>

    <!-- Notification -->
    <div class="sidebar-item" id="nav_notification" onclick="openNotifications()">
      <span>🔔 Notification</span>
      <span class="table-badge badge-green" style="font-size: 9px; padding: 1px 5px;">3 New</span>
    </div>

    <!-- Dashboard -->
    <div class="sidebar-item active" id="nav_dashboard" onclick="switchView('dashboard')">
      <span>📊 Dashboard</span>
    </div>
    ...
    <div class="sidebar-sub-list" id="setupGroup">
      ...
      <a class="sidebar-sub-item" id="nav_charges" onclick="switchView('charges')">Charges</a>
    </div>
    ...
    <!-- User Footer -->
    <div class="sidebar-user-footer" onclick="openUserMenu()">
      <div style="display: flex; align-items: center; gap: 8px;">
        <div class="user-avatar-circle">OF</div>
        <div>
          <div style="font-size: 11.5px; font-weight: 700; color: var(--text-main);">Oryx Fund Admin</div>
          <div style="font-size: 9.5px; color: var(--text-sub);">admin@oryxfund.co.ke</div>
        </div>
      </div>
  ...
  <main class="desk-main">
    ...
    <header class="desk-topbar">
      ...
      <div class="topbar-right">
        <a href="index.html" class="borrower-switch-link" title="Open Public Borrower Interface">➔ Borrower Portal</a>
  ...
  <div class="record-modal-backdrop" id="recordModal" onclick="closeRecordModal(event)">
    <div class="record-modal-box" onclick="event.stopPropagation()">
      <div class="record-modal-header">
        <h3 style="font-size: 15px; font-weight: 700;" id="recordModalTitle">Create New Record</h3>
        <span style="cursor: pointer; font-size: 16px; color: var(--text-sub);" onclick="closeRecordModalDirect()">✕</span>
      </div>
      <div class="record-modal-body" id="recordModalBody">
        <!-- Form injected dynamically -->
      </div>
      <div class="record-modal-footer">
        <button class="btn-action-sec" onclick="closeRecordModalDirect()">Cancel</button>
        <button class="btn-action-pri" onclick="saveNewRecord()">Save &amp; Submit</button>
      </div>
    </div>
  </div>

  <!-- Notification Flyout Drawer -->
  <div class="notif-drawer-backdrop" id="notifBackdrop" onclick="closeNotifications()"></div>
  <div class="notif-drawer" id="notifDrawer">
    ...
    <div style="flex: 1; overflow-y: auto;">
      ...
      <div class="notif-item">
        ...
        <div style="font-size: 12.5px; font-weight: 600; margin-top: 2px;">M-Pesa payment received: KES 17,833.00</div>
        <div style="font-size: 10.5px; color: var(--text-dim); margin-top: 4px;">18 minutes ago • Ref # QK82918239</div>
      </div>
      <div class="notif-item">
        <div style="font-size: 11px; color: var(--accent-amber); font-weight: 700;">DAILY INTEREST ACCRUAL</div>
        <div style="font-size: 12.5px; font-weight: 600; margin-top: 2px;">Interest accrual batch finished successfully</div>
        <div style="font-size: 10.5px; color: var(--text-dim); margin-top: 4px;">Today 00:00 • 100% Portfolio In Compliance</div>
      </div>
    </div>
  </div>
    ...
    function renderExecutiveDashboard(container) {{
      ...
      container.innerHTML = `
        ...
        <div class="number-cards-grid">
          ...
          <div class="number-card" onclick="switchView('loan_disbursement')">
            <div class="card-title"><span>Total Disbursed</span> <span>•••</span></div>
            <div class="card-val">Sh 750,000.00</div>
          </div>
        ...
        <!-- Row 2: 5 Operational Number Cards -->
        <div class="number-cards-grid">
          <div class="number-card" onclick="switchView('loan_application')">
            <div class="card-title"><span>New Applications</span> <span>•••</span></div>
            <div class="card-val">1</div>
          </div>
          <div class="number-card" onclick="switchView('sanctioned_loan_amount')">
            <div class="card-title"><span>Total Sanctioned</span> <span>•••</span></div>
            <div class="card-val">Sh 1.5M</div>
          </div>
          ...
          <div class="number-card" onclick="switchView('dpd_log')">
            <div class="card-title"><span>Total Shortfall</span> <span>•••</span></div>
        ...
        <div class="highlight-cards-row">
          <div class="number-card" onclick="switchView('loan_repayment')">
            <div class="card-title"><span>Total Repayments Collected</span> <span>•••</span></div>
            <div class="card-val green">Sh 50,208.33</div>
          </div>
          <div class="number-card" onclick="switchView('loan_write_off')">
            <div class="card-title"><span>Total Write Off</span> <span>•••</span></div>
            <div class="card-val red">Sh 0.00</div>
          </div>
        </div>

        <!-- Row 4: Timeline Risk Charts -->
        <div class="charts-grid-row">
          
          <div class="chart-card">
            <div class="chart-header">
              <div>
                <div class="chart-title-text">New Loans Originated</div>
                <div style="font-size: 10.5px; color: var(--text-dim); margin-top: 2px;">Real-time portfolio growth tracking</div>
              </div>
              <div style="font-size: 11px; color: var(--text-sub);">📅 Last 30 Days</div>
            </div>
            <div class="chart-canvas-mock">
              ...
              <svg class="chart-svg-line" viewBox="0 0 500 150">
                <path d="M 0 130 Q 120 110, 240 70 T 500 40" stroke="#00D26A" stroke-width="2.5" fill="none"/>
              </svg>
              <div class="chart-axis-labels">
                <span>01-Aug</span><span>07-Aug</span><span>14-Aug</span><span>21-Aug</span><span>26-Aug</span>
              </div>
            </div>
          </div>
          ...
          <div class="chart-card">
            <div class="chart-header">
              <div>
                <div class="chart-title-text">Disbursements vs Repayments</div>
                <div style="font-size: 10.5px; color: var(--text-dim); margin-top: 2px;">Liquidity and capital flow</div>
              </div>
              <div style="font-size: 11px; color: var(--text-sub);">📅 M-Pesa &amp; RTGS</div>
            </div>
            <div class="chart-canvas-mock">
              <div class="chart-grid-line" style="top: 20%;"></div>
              <div class="chart-grid-line" style="top: 50%;"></div>
              <div class="chart-grid-line" style="top: 80%;"></div>
              <svg class="chart-svg-line" viewBox="0 0 500 150">
                <path d="M 0 120 L 150 90 L 300 40 L 500 20" stroke="#34D399" stroke-width="2.5" fill="none"/>
                <path d="M 0 140 L 150 130 L 300 100 L 500 70" stroke="#60A5FA" stroke-width="2" stroke-dasharray="4" fill="none"/>
              </svg>
              <div class="chart-axis-labels">
                <span>01-Aug</span><span>07-Aug</span><span>14-Aug</span><span>21-Aug</span><span>26-Aug</span>
              </div>
        ...
        <div class="table-container-card">
          <div class="table-controls-bar">
            ...
            <div class="filter-pills-wrap">
              ...
              <button class="filter-pill" onclick="setTableFilter(this, 'recent')">Recent</button>
            </div>
          </div>
