    ...
    * { box-sizing: border-box; margin: 0; padding: 0; transition: background-color 0.15s ease, border-color 0.15s ease, color 0.15s ease; }
    ...
    @media (max-width: 991px) {
      .desk-sidebar {
        ...
        visibility: hidden;
        pointer-events: none;
        transition: transform 0.28s cubic-bezier(0.16, 1, 0.3, 1), visibility 0.28s ease, box-shadow 0.28s ease;
        box-shadow: none;
      }
      .desk-sidebar.open {
        ...
        visibility: visible;
        pointer-events: auto;
        box-shadow: 12px 0 50px rgba(0, 0, 0, 0.75);
      }
      .mobile-sidebar-close {
        display: flex;
      }
    ...
    .topbar-icon-btn:hover { background: var(--desk-card-hover); border-color: var(--desk-border-light); }
    ...
    .topbar-signout-btn:hover { background: rgba(248,113,113,0.18); }
    ...
    @media (max-width: 1200px) {
      .desk-kpi-grid { grid-template-columns: repeat(3, 1fr); gap: 10px; }
    }
    ...
    /* =========================================================================
       TABLE CARDS & RESPONSIVE TOOLBAR
       ========================================================================= */
    .desk-table-card {
      background: var(--desk-card-bg);
      border: 1px solid var(--desk-border);
    ...
    @media (max-width: 768px) {
      ...
      .table-actions .btn-action-pri {
        grid-column: span 2;
        justify-content: center;
        padding: 9px 12px;
      }
      .table-actions .btn-action-sec {
        justify-content: center;
        padding: 7px 10px;
        font-size: 11px;
      }
    ...
    .drawer-panel {
      ...
      flex-direction: column;
      transform: translateX(100%);
      transition: transform 0.25s cubic-bezier(0.16, 1, 0.3, 1), visibility 0.25s ease, box-shadow 0.25s ease;
    }
    .drawer-panel.open {
      transform: translateX(0);
      visibility: visible;
    ...
    [data-theme="light"] .drawer-panel.open {
    ...
    [data-theme="light"] .drawer-footer {
      background: var(--desk-card-surface);
      border-top-color: var(--desk-border);
    }
    ...
    [data-theme="light"] .admin-auth-modal {
      background: #FFFFFF;
      border-color: var(--desk-border);
      color: #1F3224;
    }
    [data-theme="light"] .admin-auth-modal h3 {
    ...
    @media (max-width: 640px) {
      ...
      .drawer-footer .btn-action-pri,
      .drawer-footer .btn-action-sec {
        width: 100%;
        justify-content: center;
        padding: 11px;
        font-size: 13px;
    ...
    /* =========================================================================
       IN-DOM CLEARANCE AUTH MODAL & RESTRICTED GATE
       ========================================================================= */
    .admin-auth-overlay {
      position: fixed;
      top: 0; left: 0; right: 0; bottom: 0;
...
<body>
  ...
  <div id="adminGateContainer" style="display:none; min-height: 100vh; width: 100%; background: #090909; color: #FAF8F5; align-items: center; justify-content: center; font-family: 'DM Sans', sans-serif; padding: 16px; box-sizing: border-box; position: fixed; top: 0; left: 0; z-index: 9000;">
    <div style="background: #121215; border: 1px solid #27272A; border-radius: 18px; padding: 36px 24px; max-width: 460px; width: 100%; text-align: center; box-shadow: 0 20px 60px rgba(0,0,0,0.85); box-sizing: border-box;">
      <div style="width: 60px; height: 60px; border-radius: 50%; background: rgba(220, 38, 38, 0.15); border: 1.5px solid rgba(220, 38, 38, 0.4); color: #F87171; display: flex; align-items: center; justify-content: center; margin: 0 auto 16px; font-size: 26px;">
        🔒
      </div>
      ...
      <h2 style="font-size: 20px; font-weight: 800; color: #FAF8F5; margin-bottom: 10px;">Administrator Privilege Required</h2>
      <p id="gateBorrowerDesc" style="font-size: 13px; color: #9CA3AF; line-height: 1.5; margin-bottom: 22px;">
  ...
  <aside class="desk-sidebar" id="mainSidebar">
    ...
    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; gap: 8px;">
      <a href="index.html" class="desk-brand-header" title="Oryx Fund Lending Management Platform" style="margin-bottom:0; flex:1;">
      ...
      <button class="mobile-sidebar-close" onclick="closeMobileSidebar()" aria-label="Close navigation menu" title="Close Menu">✕</button>
    </div>

    <!-- Quick Search / Command Bar Trigger -->
    <div class="sidebar-search-btn" onclick="openSearchModal()">
      <span>🔍 Search DocTypes</span>
      <span class="kbd-shortcut">Ctrl+K</span>
    </div>

    <!-- Notification Trigger -->
    <div class="sidebar-nav-item" onclick="openNotifications(); closeMobileSidebar();">
    ...
    <div class="sidebar-user-footer" onclick="logoutAdmin()" title="Click to Sign Out" style="cursor:pointer;">
      <div style="display: flex; align-items: center; gap: 8px; min-width:0; overflow:hidden;">
        ...
        <div style="min-width:0; overflow:hidden;">
          <div style="font-size: 11.5px; font-weight: 700; color: var(--text-main); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">Dervin Aziza</div>
          <div style="font-size: 9.5px; color: var(--text-sub); white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">dervinaziza9@gmail.com &bull; <span style="color:#F87171;">Sign Out</span></div>
        </div>
      </div>
      <span style="font-size: 11px; color: var(--accent-red); flex-shrink:0;">🚪</span>
    </div>
  </aside>
  ...
  <main class="desk-main">
    ...
    <header class="desk-topbar">
      <div class="topbar-left">
        ...
        <button class="mobile-menu-btn" onclick="openMobileSidebar()" aria-label="Open navigation menu" title="Menu">
        ...
        <!-- Breadcrumbs with Smart Mobile Truncation -->
        <div class="breadcrumb-wrap">
          <span onclick="switchView('dashboard')" style="cursor: pointer;" title="Home / Dashboard">🏠</span>
          <span class="breadcrumb-sep">/</span>
          <span id="topbarCategory" class="breadcrumb-cat">Dashboard</span>
          <span class="breadcrumb-sep">/</span>
      ...
      <div class="topbar-right">
        <!-- Mobile/Touch Quick Search Trigger -->
        <button class="topbar-icon-btn" onclick="openSearchModal()" title="Search DocTypes (Ctrl+K)" aria-label="Search">
          <span>🔍</span>
        </button>
        <!-- Notifications Trigger -->
        <button class="topbar-icon-btn" onclick="openNotifications()" title="Notifications" aria-label="Notifications" style="position:relative;">
          <span>🔔</span>
          <span class="topbar-badge-dot"></span>
        </button>
        <!-- Theme Toggle -->
        <button class="topbar-icon-btn" onclick="toggleAdminTheme()" id="adminThemeBtn" title="Toggle Light / Dark Mode" aria-label="Toggle Theme">☀️</button>
        <!-- Sign Out -->
        <button class="topbar-signout-btn" onclick="logoutAdmin()" title="Sign Out of Admin Desk" aria-label="Sign Out">
          <span>🚪</span> <span class="signout-label">Sign Out</span>
        </button>
      </div>
    </header>
  ...
  <!-- Global Command Palette (Ctrl+K or 🔍 Click) -->
  <div class="search-modal-backdrop" id="searchModal" onclick="closeSearchModal(event)">
    <div class="search-modal-box" onclick="event.stopPropagation()">
      <div class="search-modal-header">
        ...
        <span class="kbd-shortcut" onclick="closeSearchModalDirect()" style="cursor:pointer;">ESC</span>
      </div>
  ...
  <div class="drawer-panel" id="notifDrawer">
    <div class="drawer-header">
      ...
      <span style="cursor: pointer; font-size: 16px; padding: 4px;" onclick="closeNotifications()">✕</span>
    </div>
    <div class="drawer-body">
      <div style="background:var(--desk-card-surface); padding:12px; border-radius:8px; margin-bottom:10px; border:1px solid var(--desk-border);">
  ...
  <div class="drawer-panel" id="underwritingDrawer">
    <div class="drawer-header">
      <div>
        ...
        <h3 style="font-size: 15px; font-weight: 800;" id="undAppTitle">Application Review</h3>
      </div>
      <span style="cursor: pointer; font-size: 18px; padding: 4px;" onclick="closeUnderwritingDrawer()">✕</span>
    </div>
    ...
    <div class="drawer-footer">
      <button class="btn-action-pri" style="flex:1.5; padding:11px;" onclick="sanctionAndDisburseActiveApp()">
        <span>⚡ Sanction &amp; Disburse</span>
      </button>
      <div style="display:flex; gap:8px; flex:1.5; width:100%;">
        <button class="btn-action-sec" style="flex:1; padding:10px; justify-content:center;" onclick="requestCollateralActiveApp()">
          <span>📑 Collateral</span>
        </button>
        <button class="btn-action-sec" style="flex:1; color:#F87171; border-color:#7F1D1D; padding:10px; justify-content:center;" onclick="declineActiveApp()">
          <span>✕ Decline</span>
        </button>
      </div>
    </div>
  ...
  <div class="admin-auth-overlay" id="adminClearanceOverlay">
    <div class="admin-auth-modal">
      <div style="position:absolute; top:14px; right:14px; cursor:pointer; color:#9CA3AF; font-size:18px; width:28px; height:28px; display:flex; align-items:center; justify-content:center;" onclick="closeAdminClearanceModal()">✕</div>
      <div style="width: 52px; height: 52px; border-radius: 50%; background: rgba(0, 210, 106, 0.15); border: 1.5px solid #00D26A; color: #00D26A; display: flex; align-items: center; justify-content: center; margin: 0 auto 14px; font-size: 22px;">
        🔒
      </div>
      <div class="badge-pill" style="margin-bottom: 8px;">CLEARANCE LEVEL 4 &bull; UNDERWRITING DESK</div>
      <h3 style="font-size: 18px; font-weight: 800; color: #FAF8F5; margin-bottom: 6px;">Institutional Administrator Access</h3>
      <p style="font-size: 12.5px; color: #9CA3AF; margin-bottom: 18px; line-height: 1.45;">Enter your cryptographically verified staff credentials to decrypt and unlock institutional lending records.</p>
      ...
      <form onsubmit="handleAdminClearanceSubmit(event)">
        ...
        <button type="submit" class="btn-action-pri" style="width: 100%; padding: 12px; font-size: 13.5px; justify-content:center;" id="clearanceSubmitBtn">
          <span>⚡ Authenticate &amp; Unlock Desk</span>
        </button>

        <a href="index.html" style="display:inline-block; margin-top:14px; font-size:12px; color:#9CA3AF; text-decoration:none;">
      ...
      return `
        ...
        <div class="desk-kpi-grid" style="margin-bottom: 20px;">
          ...
          <div class="desk-kpi-card">
            ...
            <div class="kpi-value">Sh 1,000,000</div>
          </div>
          <div class="desk-kpi-card">
          ...
          <div class="desk-kpi-card">
            ...
            <div class="kpi-value">Sh 1.5M</div>
          </div>
          <div class="desk-kpi-card">
          ...
          <div class="desk-kpi-card">
            ...
            <div class="kpi-value" style="color:#34D399;">Sh 73,958</div>
          </div>
        </div>
      ...
      body.innerHTML = `
        ...
        <div class="drawer-grid-2col" style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:14px;">
          ...
          <div style="background:var(--desk-card-surface); padding:12px; border-radius:8px; border:1px solid var(--desk-border);">
            ...
            <div style="font-family:var(--font-mono); font-weight:700; color:#00D26A; margin-top:2px; font-size:13.5px;">${selectedUnderwriteApp.amount}</div>
          </div>
        </div>
        ...
        <div style="background:rgba(0, 210, 106, 0.08); border:1px dashed #00D26A; border-radius:10px; padding:14px; font-size:12px;">
          ...
          <div style="color:#D6DFD8; font-size:11.5px; line-height:1.4;">Applicant qualifies for immediate automated M-Pesa B2C disbursement under Policy Rule #2026-B.</div>
        </div>
      `;
